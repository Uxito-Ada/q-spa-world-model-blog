...
stored in `raw/qspa-base-h3-{1,2}gpu.json`; the normalized Q-SPA series
is `raw/qspa-base-h3-scaling.json`. The separate FastVideo Base H3 control
is recorded in `raw/fastvideo-base-h3.json` and uses SP4.

```bash
python experiments/h100-4gpu-e2e/scripts/plot_scaling.py \
  --input experiments/h100-4gpu-e2e/raw/qspa-base-h3-scaling.json \
...
- `TP2 x Ulysses SP2` for SGLang, LightX2V, and Q-SPA; FastVideo uses SP4;
...
The working LightX2V baseline is BF16 + SageAttention2. Q-SPA uses
...
Q-SPA's MiniMax-H3 documentation. Its `TP2 x Ulysses SP2` topology is also
...
The retained LightX2V, FastVideo, and Q-SPA MP4s pass the media gate: 124 decodable 1344 x 768 H.264 frames,
...
compare the official FastVideo distributed path with Q-SPA's FP8 Sol path.
The adapter video matrix uses the same ramen prompt for the four-GPU FastVideo
and Q-SPA FastH3 renders.

- FastVideo: BF16 Linear + FlashAttention 4.
- Q-SPA: FP8 Linear + FP8 Sol-Attn, `tau=1.0`, exact routing, quality-aware
