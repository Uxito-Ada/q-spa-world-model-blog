...
def plot_unified() -> None:
    ...
    for axis, (title, indices) in zip(axes, groups, strict=True):
        ...
        for point_x, point_y, framework in zip(x, local_throughput, local_frameworks, strict=True):
            marker = "*" if framework == "Q-SPA" else "o"
    ...
    legend_handles.extend([Line2D([0], [0], color="#C93F4B", marker="o", linewidth=1.6, markersize=5, label="Other framework throughput"), Line2D([0], [0], color="#C93F4B", marker="*", linewidth=1.6, markersize=10, label="Q-SPA throughput")])
