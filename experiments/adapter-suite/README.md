...
Q-SPA uses TP2 x Ulysses SP2, FP8 Linear, and FP8 Sol-Attn at `tau=1.0`.
...
the Base H3 and Q-SPA adapter records retain the longer repeated benchmark
...
- `raw/turbo-qspa-4gpu-regenerated.json` (same-prompt quality-matched render; smoothing=none)
- Q-SPA: `/data/heyang/qspa-lora-adapter-pr-artifacts/turbo_qspa_fp8_sol_4gpu.json`
...
python experiments/adapter-suite/plot_turbo.py \
  ...
  --candidate experiments/adapter-suite/raw/turbo-qspa-fp8.json \
...
4 with SP4 and FSDP resharding on H100; Q-SPA uses FP8 Linear + FP8
Sol-Attn with `tau=1.0` and TP2 x Ulysses SP2. Neither path uses CPU offload.

Raw records:

- `raw/fasth3-fastvideo-4gpu.json`
- `raw/fasth3-qspa-4gpu-ramen.json` (same-prompt matrix render)
- Q-SPA: `/data/heyang/qspa-lora-adapter-pr-artifacts/fasth3_qspa_fp8_sol_4gpu.json`
